...
class Achievements(commands.Cog):
    ...
    async def achievements(self, interaction: discord.Interaction, user: Optional[discord.Member] = None):
        ...
        log_command(str(interaction.user), interaction.user.id, "achievements", interaction.guild.name)
        ...
        if locked:
            ...
            for achievement in locked[:5]:  # Show first 5 locked
                current, goal, pct = get_user_achievement_progress(target_user.id, achievement.id)
